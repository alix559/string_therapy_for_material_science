...
# %% auto #0
__all__ = ['REPO', 'DAISY_THEME', 'daisy_hdrs', 'interface_icons', 'GRAPH_HTML', 'GRAPH_JSON', 'app', 'rt', 'IconKind', 'Icon',
           'lucide_icon', 'top_navbar', 'bottom_nav', 'chat_bubble', 'oob_chat_bubble', 'chat_area', 'graph_view',
...
_VIEW_IDS = ("chat-view", "graph-view")
...
def _set_active_nav_js(icon_id: str) -> str:
    return (
        "document.querySelectorAll('[data-btm-nav]').forEach(b=>b.classList.remove('active','text-primary'));"
        + f"const t=document.querySelector('[data-btm-nav=\"{icon_id}\"]');"
        + "if(t){t.classList.add('active','text-primary')}"
    )
...
def bottom_nav():
    items = []
    for ic in interface_icons:
        active = "active text-primary" if ic.active else ""
        view = "chat-view" if ic.id == "chat" else "graph-view"
        items.append(
            Button(
                lucide_icon(ic.icon),
                Span(ic.label, cls="btm-nav-label"),
                type="button",
                data_btm_nav=ic.id,
                cls=f"{active}".strip(),
                onclick="; ".join([_set_active_nav_js(ic.id), _show_view_js(view), "lucide.createIcons();"]),
            )
        )
    return Div(*items, cls="btm-nav btm-nav-sm bg-base-100 border-t border-base-300 z-20")
...
def main_panel():
    return Div(
        ...
        graph_view(),
        id="main-panel",
        cls="flex flex-col flex-1 min-h-0 overflow-hidden pb-16",
    )
...
def input_bar():
    return Div(
        Form(
            Div(
                Textarea(
                    ...
                    name="message",
                    rows=2,
                    cls="textarea textarea-bordered textarea-primary w-full resize-none rounded-2xl pr-14 leading-snug",
                    id="chat-input",
        ...
        id="input-bar",
        cls="fixed bottom-14 left-0 right-0 z-10 px-3 pb-2 pt-1",
    )
